...
void handleNewMessages(int numNewMessages)
{
    ...
    for (int i = 0; i < numNewMessages; i++)
    {
        ...
        if (text == "/start")
        {
            ...
            welcome += "/changeX  to change minimum distance to X (X=50/60/70/100/150/200)\n";
            welcome += "/n  to request current Minimum distance \n";
            welcome += "/f  to request current frequency \n";
            welcome += "/frequencyX  to change frequency to X (X=low/medium/high/secure)\n";
            ...
            welcome += "/mode0 to change to Day Mode \n";
            welcome += "/m to display current mode\n";
            welcome += "/autodistance to set distance automatically (Beta)\n";
        ...
        else if (text == "/change50")
        {
         ...
         else if (text == "/change60")
        {
            n = 60;
            String msg = "Minimum distance changed to 60";
            Serial.println(msg);
            bot.sendMessage(chat_id, msg, "");
        }
        else if (text == "/change70")
        {
            n = 70;
            String msg = "Minimum distance changed to 70";
            Serial.println(msg);
            bot.sendMessage(chat_id, msg, "");
        }
        ...
        else if (text == "/frequencylow")
        {
            freq = low;
            String msg = "Frequency changed to " + String(low) + "(low)";
            bot.sendMessage(chat_id, msg, "");
        }
        else if (text == "/frequencymedium")
        {
            freq = high;
            String msg = "Frequency changed to " + String(freq) + "(high)";
            bot.sendMessage(chat_id, msg, "");
        }
        else if (text == "/frequencyhigh")
        {
            freq = high;
            String msg = "Frequency changed to " + String(high) + "(high)";
            bot.sendMessage(chat_id, msg, "");
        }
        else if (text == "/frequencysecure")
        {
            freq = secure;
            String msg = "Frequency changed to " + String(secure) + "(secure)";
            bot.sendMessage(chat_id, msg, "");
        }
        else if (text == "/default")
        {
            ...
            mode = 0;
            cnt1 = cnt2 = 5;
            String msg = "Defaults restored Min distance=" + String(n) + "cm" + " freq=" + String(freq) + " buzz=" + String(d) + "ms Mode=0 count=" + String(cnt1);
            bot.sendMessage(chat_id, msg, "");
        }
        ...
        else if (text == "/autodistance")
        {
            bot.sendMessage(chat_id, "Calculating alarm distance...Pls keep the device steady", "");
            total = 0;
            for (int j = 0; j < 10; j++)
            {
                digitalWrite(trigPin, LOW);
            ...
            total /= 10;
            n = 0.7 * total;
            String dis = "Distance(auto) set to " + String(n);
            bot.sendMessage(chat_id, dis, "");
        }
